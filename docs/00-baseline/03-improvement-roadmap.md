...
| 순서 | 항목 | 이슈 | 스토리 라인 |
| --- | --- | --- | --- |
| 6 | 트랜잭션 경계 분리 (외부 호출 축출) | R-1 | 장애 격리 ①: 커넥션 풀 고갈을 부하로 재현 → 경계 재설계 |
| 7 | Resilience4j (retry + circuit breaker + fallback) | R-2 | 장애 격리 ②: 마이크로서비스 분리는 했으나 전파 차단 없었음 |
| 8 | AssetHistory 동시성 (`INSERT ... ON DUPLICATE KEY` / 낙관락 재시도) | R-3 | 동시성 인지·사전 처리 |
| 9 | 세션 거래 동시성 (`@Version` 낙관적 락) | R-4 | 동시성 ②: 잔고 lost update 재현 → 락 |
| 10 | 시세 결측 관측 가능화 (경고 로그 + 메트릭 + 폴백 정책 일관화) | R-5 | 조용한 실패를 시끄럽게 |
| 11 | 프로필/보안 설정 분리 (`ddl-auto: validate` on prod, SQL 로그 off) | R-6, I-8 | 운영/개발 설정 분리 원칙 |

## 스테이지 3 — 개선 (성능·구조)

| 순서 | 항목 | 이슈 | 스토리 라인 |
| --- | --- | --- | --- |
| 12 | 뉴스 랜덤 조회 쿼리 최적화 | I-1 | 쿼리 최적화 ①: Before/After 응답시간·쿼리 수 |
| 13 | StockPrice 반복 조회 배치화 | I-2 | 쿼리 최적화 ②: N회 조회 → 1회 |
| 14 | Redis 캐시 계층 도입 | I-3 | 캐시: 읽기 부하 p95 개선 |
| 15 | HTTP 클라이언트 통일 | I-5 | 리소스 재사용 |
| 16 | InvestmentReport 구조화 출력 | I-4 | 생성형 AI 연동 완성도: 파싱 실패율 측정 → 0% |
| 17 | 권한 체계 정비 | I-6 | 완성도 |
| 18 | Redis refresh token 키 개선 | I-7 | 멀티 디바이스 세션 |
| 19 | SimulationSession 데드코드 정리 | I-9 | 응집도 리팩토링 |
| 20 | 목록 API 페이지네이션 | I-10 | 확장성 |
| 21 | 리포트 생성 비동기화 | I-11 | UX + 자원 |
...
1. **쿼리·캐시 최적화** — 항목 12·13·14. Before/After 수치 확보가 쉬움.
2. **장애 격리 & 트랜잭션 경계** — 항목 6·7 + 스테이지 0 관측성. CRM 프로젝트 서사와 연결.
3. **생성형 AI 연동 완성도** — 항목 5·16. 방어적 설계 + 구조화 출력, 실패율 수치화.
4. **시뮬레이션 무결성** — 항목 3(lookahead 차단)·4(서버 가격 권위)·9(동시성). "정답을 미리 보여주지 않는다 / 클라이언트를 믿지 않는다"는 설계 원칙을 API 레벨에서 강제한 이야기.
