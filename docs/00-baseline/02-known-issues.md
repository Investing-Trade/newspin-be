...
| I-21 | `UserService.signIn`/`GlobalExceptionHandler` (신규 발견 — `newspin-web` 실사용 테스트 중 발견) | 로그인 실패(틀린 비밀번호/존재하지 않는 이메일)가 `AuthenticationException` 전용 핸들러 없이 catch-all로 떨어져 `500 "서버 내부 오류가 발생했습니다."`로 응답 — 클라이언트 입력 문제인데 서버 오류처럼 보임 | ✅ 해소 — `AuthenticationException` 핸들러 추가, `401` + `LOGIN_FAILED`(C104) ([라운드 2](../improvements/round2-login-error-message.md)) |
...
| S-2 | **학습-평가 사일로.** `EventStockImpact`(정답지)가 대형 이벤트 3건에만 존재 → 2,544건 뉴스 중 대부분의 일상 판단은 최종 리포트에서 채점 안 됨. "뉴스 판단 퀴즈"(개별 뉴스 정오)와 "투자 리포트"(이벤트 3건 대응)가 데이터상 연결 안 됨 | 리포트 입력에 `UserNewsProgress` + 개별 뉴스 감성 정오 이력을 포함해 "판단 정확도" 지표를 추가. 정답지는 이벤트 임팩트에만 의존하지 않도록 | ✅ 해소 — `user_news_progress` 에 판단 정오 영속화 + 리포트 프롬프트에 정확도 섹션 ([S3](../improvements/S3-learning-eval-silo.md)) |
