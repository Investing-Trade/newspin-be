...
- 유니크 제약이 있는데 동시성 제어가 없으면 정상 흐름에선 안 보이다가 더블클릭 한 번에 500이 난다.
  잔고 lost update 는 더 조용해서 로그도 안 남고 데이터만 틀어진다.
- **낙관 vs 비관**: 경합이 세션 단위로 국소적이고 트랜잭션이 짧아 비관적 락을 선택. 낙관적 락 + 재시도는
  재시도 경계를 트랜잭션 밖으로 빼야 해서 구조가 더 복잡해진다.
