...
- **상태**: ✅ 완료 (통합 테스트 로컬 실행은 Docker Desktop 이슈로 CI 에서 검증)
- **기간**: 2026-09-09
...
- 인수인계에서 가장 먼저 확인할 것은 "이거 어떻게 띄우죠?"에 답이 있는가. 데이터·설정·스키마가 재현 가능해야 이후 개선을 논의할 수 있다.
- 시드는 합성 대신 실제 데이터를 택함: 감성 분석 학습이 핵심 기능이라 본문이 합성이면 검증이 안 된다. 시세는 과거 확정치라 1회 수집이 가능했고, 정답지 이벤트만 실제 주가 이동폭에서 파생.
