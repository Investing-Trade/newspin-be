...
- **트랜잭션 경계**: 외부 HTTP 호출이 트랜잭션 안에 있으면 그 호출이 느려질 때 DB 커넥션이 같이 묶여
  풀이 마른다. `@EntityGraph` 로 필요한 연관만 미리 로딩해 트랜잭션을 걷어냄.
- **관측 가능한 회복탄력성**: 서킷 상태·재시도 횟수를 메트릭으로 노출 → "지금 AI 서버가 죽었는지"를
  대시보드에서 확인 가능.
- **애노테이션 대신 core 직접 사용**: Spring Boot 4 최신이라 `resilience4j-spring-boot` 스타터 자동설정
  호환이 불확실. core 를 직접 데코레이트하는 게 의존도 적고 동작이 명시적.
