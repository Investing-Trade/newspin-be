...
- **클라이언트 값을 신뢰하지 않는다**: 거래 가격을 클라이언트 값 그대로 받던 것을, 서버 시세 기준
  허용 오차 검증 + 체결가는 서버가로 재설계. "완전 일치 검증이 실패해서 검증을 껐다"를 슬리피지 허용
  범위 검증으로 되살림. 허용 오차 1%인 이유: 정상 흐름에서 FE·서버는 같은 일봉 종가라 거의 0%, 1%는
  라운딩 여유이고 스테일/하드코딩 값(10~50% 오차)은 확실히 걸러진다.
- **노출 가능 데이터의 상한 = 판단 시점**: 학습 시뮬레이션에서 판단 시점 이후 주가를 조회 API가 그대로
  주고 있었음 → 조회 레이어에서 상한 강제.
- **외부 LLM은 실패한다**: 안전 필터 차단·응답 지연·일시 오류를 각각 다르게 처리(즉시 fallback vs
